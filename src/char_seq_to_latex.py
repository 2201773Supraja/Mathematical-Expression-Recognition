...
#     :return: [TexLet]
#     """
#     # res = reduce(lambda prev, cur: str(prev) + str(cur), input_tex_lets, '') # ORIGINAL
#     res = ''.join(map(str, input_tex_lets))  # UPDATED (11 Dec)
...
#     fractions = list(fractions) # ADDED (11 Dec)
...
class TexLet(object):
    ...
    def __init__(self, template, insertion_points, extra):
        ...
        import operator
        sorted_insertion_points = sorted(insertion_points.items(), key=operator.itemgetter(1))
        self.names = list(map(lambda i: i[0], sorted_insertion_points))
        self.poses = list(map(lambda i: i[1], sorted_insertion_points))
        self.tex_lets = {}
        self.extra = extra
...
def fraction_positioning(input_tex_lets):
    ...
    :return: [TexLet]
    """
    fractions = list(filter(lambda l: l.template == FRACTION, input_tex_lets))
    ...
    if len(fractions) != 0:
        ...
        def to_left(t):
            return t.extra['right'] <= fraction.extra['left']
        left = heuristic_evaluate(list(filter(to_left, input_tex_lets)))

        # find and evaluate TexLets to right of that
        def to_right(t):
            return fraction.extra['right'] <= t.extra['left']
        right = heuristic_evaluate(list(filter(to_right, input_tex_lets)))
        ...
        def to_above(t):
            return inbound(t) and t.extra['lower'] <= fraction.extra['upper']
        above_lets = list(filter(to_above, input_tex_lets))
        new_upper = most_upper(above_lets)
        above = heuristic_evaluate(above_lets)
        ...
        def to_below(t):
            return inbound(t) and fraction.extra['lower'] <= t.extra['upper']
        below_lets = list(filter(to_below, input_tex_lets))
        new_lower = most_lower(below_lets)
        below = heuristic_evaluate(below_lets)
...
def superscript_positioning(input_tex_lets):
    ...
    """
    # find center lines
    centers = list(map(lambda t: t.extra['upper'] + (t.extra['lower'] - t.extra['upper']) / float(2), input_tex_lets))
...
def square_root_positioning(input_tex_lets):
    ...
    :return: [TexLet]
    """
    square_roots = list(filter(lambda l: l.template == SQUARE_ROOT, input_tex_lets))
    ...
    if len(square_roots) != 0:
        ...
        def inbound(t):
            return not to_left(t) and not to_right(t) and t.template != SQUARE_ROOT
        inner = heuristic_evaluate(list(filter(inbound, input_tex_lets)))
...
def char_seq_to_latex(char_seq):
    ...
    for char in char_seq:
        print(f"Char: {char['char']}, Pos: {char['pos']}")

    tex_lets = list(map(lambda char: SimpleTexLet(char['char'], char['pos']), char_seq))
    res_tex_let = heuristic_evaluate(tex_lets)
    
    print(f"Final LaTeX result: {str(res_tex_let)}")
    return str(res_tex_let)
